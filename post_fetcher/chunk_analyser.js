    ...
    if (lines[i].startsWith('Response Data:')) {
      try{
        ...
        let single_reel = (data?.items).map(reel => {
          return {
            "short_code":reel?.code,
            "play_count": reel?.play_count? reel?.play_count: 'not there',
            "comment_count": reel?.comment_count ? reel?.comment_count : 'not there',
            "caption": reel?.caption?.text ? reel?.caption?.text:'not there',
            "like_count":reel?.like_count ? reel?.like_count: 'not there',
            'hashtags':reel?.caption ? (reel?.caption?.text).match(/#\w+/g) : 'not there',
            'collaborator':reel?.coauthor_producers ? reel?.coauthor_producers[0]?.full_name : 'not there',
            'user_tags':reel?.usertags ? reel?.usertags?.in[0]?.user?.username: ''
